...
class OfflineDataHandler(DataHandler):
    ...
    def parse_windows(self, window_size, window_increment, metadata_operations=None, 
                      discrete=False, multi_rate=False, sampling_rate_key='sampling_rates'):
        ...
        multi_rate: bool default=False
            Should be True if the dataset contains sensors with different sampling rates, then window_size, 
            window_increment must be in ms. Ignored if discrete = True.
        sampling_rate_key: str, default='sampling_rates'
            the key in metadat where sampling frequency is stored. Used if multi_rate is True. Ignored if discrete = True.
        
        Returns
        ----------
        If discrete=False (default):
            np.ndarray
                A np.ndarray of size windows x channels x samples. A list if multi_rate is True, windows size will varry
        ...
        if multi_rate and not discrete:
